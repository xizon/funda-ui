...
| `data` | string  | - | Incoming data, you can set the third parameter of `onSubmit`. <blockquote>Changes in the `data` value will cause the component to re-render. It will be used when the value or content does not change when switching routes and needs to re-render the component or get the request.</blockquote> |
...
| `onSubmit` | function  | - | Call a function when the modal is submitted. It returns three callback values. The first is the trigger object, the second is the closing event (a function), and the last is a string passed by the `data` attribute |
